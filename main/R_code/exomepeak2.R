...
for (cell_line in cell_lines) {
  for (down in down){
    setwd(paste0("/home/qiayi/project/coda/Deep_merip/data/bam/bam_Hela/HeLa_",down))
    # Update file names based on the current cell line
    f1_IP =  paste0(cell_line, "_IP1_",down,"down.bam")
    f2_IP = paste0(cell_line, "_IP2_",down,"down.bam")
    f3_IP = paste0(cell_line, "_IP3_",down,"down.bam")
    IP_BAM = c(f1_IP, f2_IP, f3_IP)
    
    f1_INPUT = paste0(cell_line, "_INPUT1_",down,"down.bam")
    f2_INPUT = paste0(cell_line, "_INPUT2_",down,"down.bam")
    f3_INPUT = paste0(cell_line, "_INPUT3_",down,"down.bam")
